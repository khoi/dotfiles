...
type QuestionType = "select" | "text" | "textarea";
type AnswerSource = "option" | "custom" | "mixed" | "text";
...
function errorResult(message: string, questions: AskQuestion[] = []): { content: { type: "text"; text: string }[]; details: AskResult } {
	return {
...
function normalizeQuestions(rawQuestions: AskQuestionInput[]): AskQuestion[] {
...
function formatSummary(questions: AskQuestion[], answers: AskAnswer[]): string {
	if (answers.length === 0) {
...
export default function ask(pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
						...
						if (question.type === "select") {
							...
							for (let index = 0; index < options.length; index += 1) {
								...
								const checked = option.isOther
									? !!customValue
									: selected.has(option.baseIndex ?? -1);
			...
			return {
				content: [{ type: "text", text: formatSummary(questions, result.answers) }],
				details: result,
			};
		},

		renderCall(args, theme) {
			const questions = ((args.questions as AskQuestionInput[]) || []).map((question, index) => question.label || question.id || `Q${index + 1}`);
