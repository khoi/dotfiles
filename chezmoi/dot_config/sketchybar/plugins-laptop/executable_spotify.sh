...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME icon.color=$COLOR_SPOTIFY_PAUSED label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes icon.color=$COLOR_SPOTIFY_PLAYING

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME icon.color=$COLOR_SPOTIFY_PAUSED
